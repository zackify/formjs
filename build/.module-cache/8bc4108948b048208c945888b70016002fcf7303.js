...
var initialize = React.createClass({displayName: 'initialize',
  ...
  handleSubmit: function() {
    var data = [];
    var refs = this.state.data.map(function (ref) {
      var name = ref.name;
      var value = this.refs.submit.getDOMNode().value();
      data.push(value)
    });
  $.ajax({
  ...
  render: function() {
    ...
    return(
      React.DOM.form( {onSubmit:this.handleSubmit}, 
        elements,
        React.DOM.input( {type:"submit", ref:"submit", value:this.state.form.submitText} )
      )
    );
  }
});
...
var generateInputField = React.createClass({displayName: 'generateInputField',
  ...
  render: function(){
    return(
      ...
      React.DOM.input( {type:this.props.type, ref:this.props.name, placeholder:this.props.placeholder, value:this.props.value})
      )
    );
  }
});
